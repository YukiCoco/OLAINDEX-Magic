        ...
        <div class="card-body">
            <form action="{{ route('apply') }}" method="get" target="_blank">
                <div class="form-group">
                    ...
                    <span class="form-text text-danger">申请此处必须为Https，程序会根据你当前设置的链接显示。如已申请，请直接在下面配置中填写；也可使用 https://olaindex.ningkai.wang 中转。</span>
                    <span class="form-text text-danger"><b>注：此申请流程仅支持国际版OneDrive，世纪互联版需单独申请。</b></span>
                </div>
        ...
        <div class="card-body">
            <form action="" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="redirect_uri" name="redirect_uri"
                           value="{{ trim(config('app.url'),'/').'/oauth' }}">
                    <span class="form-text text-danger">确保回调地址格式为此形式 https://you.domain/oauth，使用中转域名无需https协议（注意：如果通过CDN开启HTTPS而非配置SSL证书，部分回调CDN会跳转http地址，从而导致申请失败） </span>
                </div>
                <div class="form-group">
